...
class RealmToDoListViewController: UIViewController {
    ...
    @IBAction func addTaskButton(_ sender: Any) {
        guard let vc = storyboard?.instantiateViewController(identifier: "taskManager") as? RealmToDoListTaskMakingViewContrloller else {
            return
        }
...
extension RealmToDoListViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        guard let vc = storyboard?.instantiateViewController(identifier: "Delete") as? RealmToDoListCompletedTaskViewController else {
            return
        }
        vc.item = item
